...
function getControllers() {
  return {
    ...
    subscriptionController: require(path.join(
      config.controllers,
      'subscription.js'
    )),
...
module.exports = function (app, options) {
  ...
  if (!config)
    throw new Exception(
      'Options must be passed to the appdirect-express module'
    );
  if (!app)
    throw new Exception(
      'An express app must be passwed to the appdirect-express module'
    );

  if (config.use_mocks)
    config.controllers = path.join(__dirname, '/mock_controllers');
  else if (!config.controllers)
    throw new Exception(
      'You must provide a controllers directory when use_mocks is off'
    );
  ...
  baseApi.use(
    '/subscription',
    require('./routes/subscription.js')(
  ...
  baseApi.use(
    '/user',
    require('./routes/user.js')(controllers.userController, config)
  );
  baseApi.use(
    '/addon',
    require('./routes/addon.js')(controllers.addonController, config)
  );
  app.use('/api', baseApi);
